...
class Block:
    ...
    def hash_block(self):
        ...
        # Turn the record into an encoded string
        record = str(self.record).encode()
        # Add the encoded record into the hash
        sha.update(record)

        # Turn the creator_id into an encoded string
        creator_id = str(self.creator_id).encode()
        # Add the encoded creator_id into the hash
        sha.update(creator_id)

        # Encode the time of record creation
        timestamp = str(self.timestamp).encode()
        # Add the encoded record time into the hash
...
# which is set equal to a `Record` that contains the `sender`, `receiver`, and `amount` values. The updated `Block`should also include 
# the attributes for `creator_id` and `prev_hash`.
